...
class SupplyState(object):
    ...
    def get_product_company(self, id):
        address = addresser.get_product_address(id)
        LOGGER.info("1231")
        state_entries = self._context.get_state(
            addresses=[address], timeout=self._timeout)
        LOGGER.info("12322221")
        if state_entries:
            container = product_pb2.ProductCompanyContainer()
            LOGGER.info("1221312331")
            container.ParseFromString(state_entries[0].data)
            LOGGER.info("122asdasd2331")
            for product in container.entries:
                if product.id == id:
                    return product
    ...
    def get_product_employee(self, id):
        address = addresser.get_product_employee_address(id)
        state_entries = self._context.get_state(
            addresses=[address], timeout=self._timeout)
        if state_entries:
    ...
    def update_company(self,
        ...
        product_address = addresser.get_product_address(id)
        company = product_pb2.Product_company.Company(
            timestamp = timestamp,
            address = address,
            price_IPO = price_IPO
        )
        ...
        state_entries = self._context.get_state(
            addresses=[product_address], timeout=self._timeout)
        if state_entries:
            LOGGER.info(state_entries[0].data)
            container.ParseFromString(state_entries[0].data)
    ...
    def update_employee(self,
        ...
        product_address = addresser.get_product_address(id)
        employee = product_pb2.Product_employee.Employee(
            timestamp = timestamp,
            position = position,
            salary = salary
        )
        LOGGER.info("company123")
        container = product_pb2.ProductEmployeeContainer()
        state_entries = self._context.get_state(
            addresses=[product_address], timeout=self._timeout)
        if state_entries:
            container.ParseFromString(state_entries[0].data)
            LOGGER.info("data1w23")
            for product in container.entries:
                if product.id == id:
